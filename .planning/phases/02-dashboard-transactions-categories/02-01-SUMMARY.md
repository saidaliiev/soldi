...
- **DonutChart per-slice path interpolation across distinct topologies (D-05 strict)** — current crossfade preserves the 300ms `Easing.out(Easing.cubic)` feel but does not interpolate individual arc segments. `usePathInterpolation` from Skia requires matched path command structure across keyframes; that requires same-N-slice transitions which a 6→5→7-slice month change does not satisfy without padding. Plan 02-02 will revisit when adding the digest card (same shared-value plumbing).
