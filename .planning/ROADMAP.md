...
- [~] **Phase 5: Polish + TestFlight Beta** — Settings, biometric, notifications, performance, Internal TestFlight launch (2026-05-16 session 2: 05-01/02/03 code-complete; 05-04 ALL tasks DONE — P0 #3/#4 cleared, build #4 `a29159a3` v1.0.0 uploaded to App Store Connect TestFlight [ASC app `6770086922`, submission `86769705-...`], 7 forward-only commits incl. real build-blocker fixes [op-sqlite plugin, chat-empty Metro resolve, Sentry upload]. Mark [x] after Apple processing completes + ≥1 internal tester runs it on device [D-04 bar: cold launch + biometric gate + dashboard]. Device UAT pending P0 #1.)
...
- [x] 04-02: Round-up calculation engine (rules: round-up, fixed weekly, percentage of income)
- [x] 04-03: Full Ukrainian translation pass via Claude + Upwork native reviewer

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 04-04: Accessibility audit + fixes (accessibilityLabel/role, dynamic type, contrast, VoiceOver order)
